...
class VerifyUserAPI(MethodView):
    def get(self,token):
        ...
        email = UserMethods.decode_verification_token(token)
        ...
        if user:
            ...
            return make_response(jsonify({'status': 'error',
                                          'message': 'The account verification link is either expired or invalid. \
                                                      Please try to login to verify the account.',
                                          'data': None}), 404)
